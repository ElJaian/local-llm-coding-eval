...
run_bench () {
  local name="$1" m="$2"
  echo "######## $name :: $m ########" | tee -a "$OUT/table.txt"
  ...
  "$BIN" -m "$m" -ngl 99 -fa 1 -ctk q8_0 -ctv q8_0 \
     -p 512 -p 2048 -n 128 -d 0,4096,16384,32768 -r 3 2>>"$OUT/$name.err" | tee -a "$OUT/table.txt" || true
}
...
for ctx in 4096 16384 32768; do
  python3 - "$ctx" <<'PY' | tee -a "$OUT/table.txt"
...
req=urllib.request.Request("http://172.28.192.1:11434/api/generate",data=body,
...
echo "BENCH_SPEED_CONTEXT_DONE  $(cat "$OUT/ctxfit.txt" 2>/dev/null | tr '\n' ' ')"
